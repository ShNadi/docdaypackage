# `docdaypackage` developer documentation
...
# install docdaypackage as an editable package
...
The second is to use `tox`, which can be installed separately (e.g. with `pip install tox`), i.e. not necessarily inside the virtual environment you use for installing `docdaypackage`, but then builds the necessary virtual environments itself by simply running:
...
cd $(mktemp -d docdaypackage.XXXXXX)
git clone git@github.com:shnadi/docdaypackage .
...
Visit
[https://test.pypi.org/project/docdaypackage](https://test.pypi.org/project/docdaypackage)
and verify that your package was uploaded successfully. Keep the terminal open, we'll need it later.

In a new terminal, without an activated virtual environment or an env directory:

```shell
cd $(mktemp -d docdaypackage-test.XXXXXX)
...
--extra-index-url https://pypi.org/simple docdaypackage
```
...
Don't forget to also make a [release on GitHub](https://github.com/shnadi/docdaypackage/releases/new).GitHub-Zenodo integration will also trigger Zenodo into making a snapshot of your repository and sticking a DOI on it.